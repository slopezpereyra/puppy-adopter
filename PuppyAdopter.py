...
def save_pages_in_data(saves_list): #This function creates a key value "saves" holding the "saves_list" list, 
    # where the downloaded url will be stored, all in the saves.dat file.
...
def look_images(pages_list, saves_list): #This is where the magic happens!!
    ...
    for i in pages_list:
        ...
        if str(image_url).endswith('.jpg 2x'):
            image_url = str(image_url.replace(' ', '')[:-2]) #This if statement responds to the fact that sometimes some 
            # files' format ended with " 2x" (f.e., ".jpg 2x"), and the file was broken. Now if the url ends with " 2x", 
            # that will be erased, leaving only the ".jpg".
        if not str(image_url).startswith("http"): 
            image_url = "https://" + str(image_url) # If missing, adds the initial https string to the url.
        response = requests.get(image_url, stream=True)
        response.raise_for_status()
        saves_list.append(str(image_url)) #Adds the requested url to the saves_list, which is later added to the saves.dat file, 
        # to store the already downloaded url's.
        download_image(image_url, response)
        print('¡Cachorrito adoptado!')

def get_page():  #This method holds all the other methods together, apart from stating the default web sites to scrap 
# and get the images from.
    ...
    alr_dow = [] #alr_dow stands for "already downloaded"; in this list, the downloaded url's will be appended 
    # (see line 123, look_images function).
